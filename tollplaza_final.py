...
app = FastAPI(title="Toll Plaza AI", version="1.0.0")
...
def log_detection(vehicle_type, confidence, license_plate=""):
    conn = sqlite3.connect('tollplaza.db')
    c = conn.cursor()
    now = datetime.now()
    c.execute('''INSERT INTO detections 
                 (timestamp, vehicle_type, confidence, license_plate, hour, day_of_week, date)
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (now, vehicle_type, confidence, license_plate, now.hour, now.weekday(), now.date()))
    conn.commit()
...
<head>
    ...
    <style>
        ...
        .demo { text-align: center; margin-top: 20px; color: #666; font-size: 12px; }
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .preview-img { max-width: 150px; margin: 5px; border-radius: 5px; }
        table {
            width: 100%;
...
<body>
    ...
    <script>
        ...
        async function detectVehicles() {
            ...
            let html = `<div style="background:#d4edda; padding:15px; border-radius:8px; margin-top:15px;">
        ...
        async function loadRecentDetections() {
            ...
            const data = await response.json();
            let html = '<table><tr><th>Time</th><th>Vehicle</th><th>Confidence</th></tr>';
            data.forEach(d => {
                html += `<tr>
...
async def login(username: str = Form(...), password: str = Form(...)):
    ...
    if user:
        ...
        return response
    return HTMLResponse(content=LOGIN_PAGE.replace('<div class="demo">', '<div class="error">Invalid credentials</div><div class="demo">'))
